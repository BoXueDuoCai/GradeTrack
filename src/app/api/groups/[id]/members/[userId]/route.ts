...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string; userId: string }> }) {
  try {
    ...
    if (group.creatorId !== user.id && user.role !== 'super_admin') {
      return NextResponse.json({ success: false, error: '只有创建者或超管能移除成员' }, { status: 403 })
    }

    await db.groupMember.deleteMany({ where: { groupId: id, userId } })
    return NextResponse.json({ success: true })
  } catch (e) {
    console.error('DELETE /api/groups/[id]/members/[userId] error:', e)
    return NextResponse.json({ success: false, error: '移除成员失败' }, { status: 500 })
  }
}
